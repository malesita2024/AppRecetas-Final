...
window.addEventListener('DOMContentLoaded', async () => {
    try {
        let respuesta = await fetch('js/recetas.json');
        let data = await (respuesta).json();
        recetas = data;
        loadRecetasPopulares(recetas);
...
if (window.location.pathname == "/index.html") {
    ...
    Swal.fire({
        title: "Bienvenido",
        text: "Disfruta nuestra App de recetas!",
        imageUrl: "https://img.freepik.com/vector-gratis/concepto-receta-cafe-dalgona_23-2148565215.jpg?t=st=1713746427~exp=1713750027~hmac=8ae695f1814fa4ecfa1d2a73645f2fe62b5e14c8e5660d55f57d2c8cd1c4c7c4&w=740",
        imageWidth: 300,
        imageHeight: 300,
...
function leerDatosProducto(item) {
    let valorLocalStorage = localStorage.getItem('recFavoritas');

    if (valorLocalStorage === null || valorLocalStorage === "") {
        recetasFavoritas = [];
    } else {
        recuperarRecetasFavoritas = localStorage.getItem('recFavoritas');
        recetasFavoritas = JSON.parse(recuperarRecetasFavoritas);
    }
